...
class CharbonnierLoss(nn.Module):
    """
    Charbonnier Loss (L1-L2 loss)
    Formula: sqrt(x^2 + epsilon^2)
    where x is the difference between predicted and target values.
    """
    ...
    def forward(self, input, target):
        """
        Calculates the Charbonnier Loss between two tensors.

        Args:
            input (torch.Tensor): The predicted tensor (e.g., predicted image).
            target (torch.Tensor): The target tensor (e.g., ground truth image).

        Returns:
            torch.Tensor: The computed Charbonnier Loss.
        """
        if not (input.shape == target.shape):
            raise ValueError(f"Input and target tensors must have the same shape. Got {input.shape} and {target.shape}")

        diff = input - target
        # Calculate sqrt(diff^2 + epsilon^2)
        ...
        if self.reduction == 'mean':
            ...
            raise ValueError(f"Reduction method '{self.reduction}' not supported. Choose from 'mean', 'sum', or 'none'.")
...
def train(nb_epoch, batch_size, store_name, num_class=0, start_epoch=0, data_path=''):
    ...
    model.head.fc = torch.nn.Linear(2048, 396)
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader)):
            ...
            enable_running_stats(net)
            optimizer.zero_grad()
            output_1_final, output_2_final, output_3_final, output_ORI, _, _, _ = net(inputs)
        ...
        val_acc_com, val_loss = test(net, CELoss, 3, data_path + '/valid')
...
if __name__ == '__main__':
    ...
    train(nb_epoch=200,  # number of epoch
          ...
          num_class=396,  # number of categories
